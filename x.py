...
# Cấu hình trình duyệt headless và ẩn danh
chrome_options = Options()
chrome_options.add_argument('--headless')  # Chạy headless
chrome_options.add_argument('--no-sandbox')
chrome_options.add_argument('--disable-dev-shm-usage')
chrome_options.add_argument('--incognito')  # Chạy trong chế độ ẩn danh
...
def load_random_page():
    ...
    while True:
        # Kiểm tra nếu đã qua 5 phút (300 giây)
        if time.time() - start_time >= 5 * 60:
            print("Đã hết thời gian chạy 5 phút. Kết thúc chương trình.")
            break
        ...
        # Tạo thời gian ngẫu nhiên để tạm dừng giữa 1 đến 2 phút (60 giây đến 120 giây)
